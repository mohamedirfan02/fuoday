...
class AssignedPersonDropdownCheckboxState
  ...
  // Helper method to get access-based employee list
  List<EmployeeModelEntity> _getEmployeesBasedOnAccess(
    EmployeeDepartmentEntity? employeeDepartment,
  ) {
    final hiveService = getIt<HiveStorageService>();
    final employeeDetails = hiveService.employeeDetails;

    if (employeeDetails == null || employeeDepartment == null) {
      return [];
    }
    ...
    // Convert comma-separated roles into a list
    final accessList = rawAccess.split(',').map((e) => e.trim()).toList();

    // Management access - can see all employees
    if (accessList.contains('management')) {
      return employeeDepartment.data;
    }

    // HR access - can see all employees
    if (accessList.contains('hr')) {
      return employeeDepartment.data;
    }

    // Manager access - can see employees in same department
    if (accessList.contains('manager')) {
      return employeeDepartment.sameDepartment;
    }

    // Employee access - can see employees in same exact department
    if (accessList.contains('employee')) {
      return employeeDepartment.sameEmpDepartment;
    }

    // Default: return empty list if no recognized access
    return [];
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<EmployeeDepartmentProvider>(
      builder: (context, provider, _) {
        ...
        final employees = _getEmployeesBasedOnAccess(
          provider.employeeDepartment,
        );
